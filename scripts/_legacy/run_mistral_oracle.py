...
# ============================
# Mistral oracle (via Mistral AI API)
# ============================
...
class MistralOracle:
    ...
    async def label(self, *, cache_key: str, item_text: str, k_classes: int) -> OracleReply:
                    ...
                    async with aiohttp.ClientSession(timeout=timeout) as session:
                        async with session.post(self.api_url, headers=headers, json=payload) as resp:
                            ...
                            if resp.status < 200 or resp.status >= 300:
                                raise RuntimeError(f"Mistral HTTP {resp.status}: {text_body[:500]}")
        ...
        raise RuntimeError(f"Mistral oracle failed after retries: {last_err}")
...
# ============================
# Stream simulator (with Mistral oracle)
# ============================
...
async def simulate_datasets(
    *,
    ...
    oracle = MistralOracle(concurrency=oracle_concurrency) if use_mistral_oracle else MistralOracle(concurrency=1)
    ...
    for ds in datasets:
        ...
        for s in range(seeds):
            tasks.append(asyncio.create_task(
                simulate_one_run(dataset=ds.name, seed=s, params=p, oracle=oracle, use_mistral_oracle=use_mistral_oracle)
            ))
...
async def main() -> None:
    ...
    df = await simulate_datasets(
        ...
        use_mistral_oracle=USE_MISTRAL_ORACLE,
